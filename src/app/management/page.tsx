...
export default function ManagementPage() {
  ...
  const { loading: currentUserLoading, error: currentUserError, data: currentUserData } = useQuery(GET_CURRENT_USER, {
  ...
  const { loading: usersLoading, error: usersError, data: usersData, refetch: refetchUsers } = useQuery(GET_ALL_USERS, {
    skip: !currentUser || !['SUPER_ADMIN', 'ADMIN'].includes(currentUser.role),
    fetchPolicy: 'network-only',
  });

  const { loading: questionsLoading, error: questionsError, data: questionsData, refetch: refetchQuestions } = useQuery(GET_ALL_QUESTIONS, {
    fetchPolicy: 'network-only',
  });
  ...
  if (!currentUser) return <p>No user data available. Please try logging in again.</p>;
      ...
      {canManageUsers && (
        <section className="mb-8">
          ...
          >
            Manage Users
            <svg 
              className={`ml-2 h-5 w-5 transform ${isUserSectionCollapsed ? 'rotate-0' : 'rotate-180'}`} 
...
{canManageQuestions && (
        <section className="mb-8">
          ...
          >
            Question Management
            <svg 
              className={`ml-2 h-5 w-5 transform ${isQuestionSectionCollapsed ? 'rotate-0' : 'rotate-180'}`} 
              xmlns="http://www.w3.org/2000/svg" 
              viewBox="0 0 20 20" 
              fill="currentColor"
            >
              ...
              <tbody>
                {questionsData?.questions.map((question: any) => (
                  <tr key={question.id}>
                    ...
                    <td className="border border-gray-300 p-2">
                      {editingQuestion?.id === question.id ? (
                        <input
                          ...
                          onChange={(e) => setEditingQuestion({...editingQuestion, answers: e.target.value.split(', ')})}
                          className="w-full p-1 border rounded"
                        />
                      ) : (
                    ...
                    <td className="border border-gray-300 p-2">{question.createdBy.username}</td>
                    <td className="border border-gray-300 p-2 text-center">
                      {editingQuestion?.id === question.id ? (
                        <button
